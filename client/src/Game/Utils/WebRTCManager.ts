...
export class WebRTCManager {
	...
	constructor(private sessionId: string, private stunServers: RTCIceServer[] = [], room?: any) {
		if (room) {
			this.room = room;
		}
	}
	...
	async initialize() {
		...
		if (this.room) {
			this.room.onMessage("media-state-change", (message:any) => {
				...
				this.handleRemoteMediaStateChange(peerId, videoEnabled, audioEnabled);
			});
		}
	}
	...
	private handleRemoteMediaStateChange(peerId: string, videoEnabled: boolean, audioEnabled: boolean) {
		...
		if (videoEl) {
			...
			if (videoWrapper) {
				...
				let videoOffIndicator = document.getElementById(`video-off-indicator-${peerId}`);
				if (!videoEnabled) {
				...
				const audioOffIcon = document.getElementById(`audio-icon-${peerId}`);
				if (audioOffIcon) {
					audioOffIcon.style.display = audioEnabled ? "none" : "block";
				}
			}
		}
	}
	...
	private createVideoOffIndicator(peerId: string, wrapper: HTMLElement) {
		...
		videoOffIndicator.innerHTML = '<div class="avatar-placeholder">👤</div>';
	...
	initiatePeerConnection(remotePeerId: string) {
		if (!this.localStream || !this.peer) {
			console.error('Cannot initiate peer connection: no local stream or peer');
			return;
		}
		...
		try {
			...
			call.on('error', (err) => {
				...
				// Add retry logic
				if (!this.retryConnections[remotePeerId] || this.retryConnections[remotePeerId] < 3) {
					this.retryConnections[remotePeerId] = (this.retryConnections[remotePeerId] || 0) + 1;
					console.log(`Retrying connection to ${remotePeerId}, attempt ${this.retryConnections[remotePeerId]}`);
					setTimeout(() => {
			...
			console.error(`Error creating peer connection to ${remotePeerId}:`, error);
		}
	}
	...
	addVideoElement(peerId: string, stream: MediaStream, isLocal: boolean) {
		...
		videoElement.classList.add('video-element', isLocal ? 'local-video' : 'remote-video');
		...
		nameTag.textContent = peerId;  // You can replace peerId with player name if available
		...
		videoOffButton.textContent = this.videoEnabled ? 'Video Off' : 'Video On';
	...
	private toggleVideo(stream: MediaStream, peerId: string, sendUpdate = true) {
		...
		if (peerId === this.sessionId) {
			...
			let indicator = document.getElementById(`video-off-indicator-${peerId}`);
			
			// Update the button text
			const videoOffButton = document.querySelector(`#video-wrapper-${peerId} .video-control-btn`);
			if (videoOffButton) {
				videoOffButton.textContent = this.videoEnabled ? 'Video Off' : 'Video On';
			}
			
			// Make sure the controls container remains visible
			const controlsContainer = document.querySelector(`#video-wrapper-${peerId} .controls-container`);
	...
	private toggleAudio(stream: MediaStream, peerId: string, sendUpdate = true) {
	...
	private updateVideoLayout() {
		...
		container.classList.remove('one-participant', 'two-participants', 'three-participants', 'four-participants', 'many-participants');
		...
		Object.values(this.videoElements).forEach((videoElement) => {
			...
			if (wrapper) {
				wrapper.style.display = participantCount > 3 && !this.isGridView ? 'none' : 'block';
			}
		});
	}
	...
	shutdown() {
		...
		Object.keys(this.videoElements).forEach(this.removeVideoElement.bind(this));
		this.videoElements = {};
		...
		const extraButtons = document.querySelectorAll('.video-control-btn, .audio-control-btn');
